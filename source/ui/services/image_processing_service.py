...
def load_annotations():
    if os.path.exists(CSV_ANNOTS_PATH):
        df = pd.read_csv(CSV_ANNOTS_PATH)
        return df
    else:
        st.error(f"Annotations file not found at {CSV_ANNOTS_PATH}")
        return pd.DataFrame()
...
def get_chroma_collection():
    ...
    try:
        ...
        st.error(f"Error loading ChromaDB collection: {e}. Please ensure the ChromaDB is properly initialized.")
        return None
...
def process_crops_for_streamlit(uploaded_image_pil, df_annotations_for_this_image):
    ...
    for idx, row in df_annotations_for_this_image.iterrows():
        ...
        try:
            ...
            crop_id = f"{Path(original_image_name).stem}_bbox_{idx}"
            ...
            st.warning(f"Error processing crop {idx} from {original_image_name}: {e}")
    return pd.DataFrame(crop_data)
...
def load_model_and_transforms():
    ...
    if os.path.exists(WEIGHTS_PATH):
        try:
            encoder_model.load_state_dict(torch.load(WEIGHTS_PATH, map_location=device))
            st.success(f"Loaded custom weights") #from {WEIGHTS_PATH}")
        except Exception as e:
            st.error(f"Error loading model weights: {e}. Using baseline weights.")
    else:
        st.warning(f"No custom checkpoint found at {WEIGHTS_PATH}. Executing using baseline weights.")
...
def get_embedding_resnet(input_img):
    if encoder is None or inference_transform is None or device is None:
        st.error("Model or transforms not loaded. Cannot generate embeddings.")
        return None
...
def classify_image(image_bytes: BytesIO) -> ImageClassifications:
    ...
    df_annotations_for_this_image = df_annots_global[df_annots_global['image'] == original_image_pil]
    
    if df_annotations_for_this_image.empty:
        #display st msg
        st.error(f"No annotations found for '{original_image_name}' in the dataset. Please upload an image only from the given subset.")
    else:
        ...
        if not df_test_crops_streamlit.empty:
            #display st msg
            st.success(f"Extracted {len(df_test_crops_streamlit)} product crops.")
            # Step 2: recognizing the extracted crops
            st.write("Analysing the products...")
            ...
            for _, row in df_test_crops_streamlit.iterrows():
                if row['crop_pil'] is not None: # Ensure crop_pil is not None
                    embedding = get_embedding_resnet(row['crop_pil'])
            ...
            if not df_test_crops_streamlit.empty and collection is not None:
                st.write("Classfying the products...")
                ...
                if query_embeddings_for_db:
                    ...
                    if not df_test_crops_streamlit.empty:
                        annotated_image = draw_boxes_on_image(image_pil, df_test_crops_streamlit, df_classnames)
                        ...
                        st.warning("No product classifications were obtained. This might indicate that no matching products were found or the products are unknown.")
                else:
                    st.warning("Products are not recognized.")
                
            else:
                if collection is None:
                    st.error("Database is not available. Please check the DB setup.")
                else:
                    st.warning("No products are recognized from the image.")
        else:
            st.warning("Image is unidentified / Not known to model.")
